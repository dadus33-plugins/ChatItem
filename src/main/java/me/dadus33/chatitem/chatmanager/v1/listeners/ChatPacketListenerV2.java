...
import me.dadus33.chatitem.chatmanager.v1.packets.PacketMetadata;
		...
		Bukkit.getScheduler().runTaskAsynchronously(ChatItem.getInstance(), () -> {
			...
			try {
				if (!p.getItemInHand().getType().equals(Material.AIR)) {
					...
					message = manager.getManipulator().parse(json, getStorage().PLACEHOLDERS, copy, styleItem(copy, getStorage()),
							manager.getPlayerVersionAdapter().getProtocolVersion(p));
				} else {
					if (!getStorage().HAND_DISABLED) {
						message = manager.getManipulator().parseEmpty(json, getStorage().PLACEHOLDERS, getStorage().HAND_NAME,
								getStorage().HAND_TOOLTIP, p);
					}
				}
			} catch (Exception e1) {
				e1.printStackTrace();
