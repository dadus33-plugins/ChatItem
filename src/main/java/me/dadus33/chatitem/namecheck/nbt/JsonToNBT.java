...
public class JsonToNBT
{
    ...
    private static Any nameValueToNBT(String key, String value) throws NBTException
    {
        ...
        if (value.startsWith("{"))
        {
            ...
            Compound jsonNbtCompound;
            String s1;

            for (jsonNbtCompound = new Compound(key); value.length() > 0; value = value.substring(s1.length() + 1))
            {
                s1 = nextNameValuePair(value, true);

                if (s1.length() > 0)
                {
                    jsonNbtCompound.tagList.add(getTagFromNameValue(s1, false));
                }
        ...
        else if (value.startsWith("[") && !INT_ARRAY_MATCHER.matcher(value).matches())
        {
            ...
            for (jsonNbtList = new List(key); value.length() > 0; value = value.substring(s.length() + 1))
            {
                s = nextNameValuePair(value, false);

                if (s.length() > 0)
                {
                    jsonNbtList.tagList.add(getTagFromNameValue(s, true));
                }
    ...
    {
        java.util.List<Any> tagList = Lists.newArrayList();
    ...
    static class List extends Any
    {
        ...
        public NBTBase parse() throws NBTException
        {
            ...
            for (Any jsonNbtAny : this.tagList)
            {
                nbttaglist.appendTag(jsonNbtAny.parse());
            }
